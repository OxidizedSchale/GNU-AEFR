...
# AEFR 目前并不是一个为了讨好所有人而设计的复杂应用，因为仍未做出完整版本。它目前只是 1102 行直达底层的 Rust 逻辑所构成的一个内核。在计算机的世界里，最短的路径永远是最无敌的！如果你追求的是目前的兼容性和易用，请去用 AA；如果你追求的是极致的自由与性能以及社区维护，欢迎来到 AEFR
...
​在 AEFR v0.8+ 中，我们引入了自主研发的“绅士调度器”。我们不信任操作系统的默认调度，因为它们往往为了所谓的“公平”而牺牲了实时渲染的确定性。

​核心准则：N-2 策略

​AEFR 拒绝做吞噬 CPU 的贪婪野兽。调度器会自动探测物理核心数 N，并强行隔离出 N-2 个核心作为计算专区（在只有两个核心/线程的情况下，AEFR会占一个）

​1 核心预留给 UI/Render (Main Thread)：确保在计算量暴涨时，渲染帧率依旧丝滑

​1 核心预留给 OS/Audio (Backstage)：作为底层系统的“避震空间”，彻底杜绝背景音乐因 CPU 满载而产生的爆音与抖动

​其余核心分配给 AEFR Worker：利用 Rayon 线程池进行 Spine 骨骼蒙皮与物理计算
...
​禁止在任何计算密集型任务中使用 std::thread::spawn。所有并行任务必须通过 AefrScheduler 进行分发，违者将导致系统调度陷入无序竞争，这是对 AEFR 的 Rust 美学进行背叛
